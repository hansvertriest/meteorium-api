...
// Types
import { IObservationWithShower, IObservationWithShowerLegacy, IShowerAtLocation, IShowerAtLocationLegacy } from '../../d.types';
...
export const convertDateToLegacy = (date: Date): string => {
  ...
  const dateString = date.toString().replace('+0000 (Greenwich Mean Time)', '');
  const dateStringComps = dateString.split(' ');
  return (
    dateStringComps[0] + ', ' + dateStringComps[2] + ' ' + dateStringComps[1] + ' ' + dateStringComps[3] + ' ' + dateStringComps[4] + ' ' + dateStringComps[5]
  );
};
